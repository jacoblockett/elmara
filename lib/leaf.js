import { innerText, textContent, getSiblings, getParent } from "domutils"
...
class Leaf {
	 ...
	 */
	constructor(node = {}) {
		if (!isParentNode(node) && Object.prototype.toString.call(node) !== "[object Object]") {
	 ...
	 *
	 * @param {{convert: boolean, skipText: boolean}} options
	 * @param {boolean} [options.convert] If true, converts found children to Leaves (default = true)
	 * @param {boolean} [options.skipText] If true, skips text nodes (default = false)
	 * @returns {any[]}
	 */
	#getChildren(options = { convert: true, skipText: false }) {
		if (Object.prototype.toString.call(options) !== "[object Object]")
			options = { convert: true, skipText: false }

		if (typeof options.convert !== "boolean") options.convert = true
		if (typeof options.skipText !== "boolean") options.skipText = false
		...
		for (let i = 0; i < children.length; i++) {
			...
			if (child instanceof Element || (child instanceof Text && !options.skipText)) {
				found[found.length] = options.convert ? new Leaf(child) : child
			}
		}
	 ...
	 */
	get innerMarkup() {
		return serializeDOM(this.#getChildren({ convert: false }))
	}
	...
	/**
	 * Sets the text of Leaf.
	 *
	 * Same as {@link setText}
	 *
	 * @param {String} newText
	 * @returns {Leaf}
	 */
	set text(newText) {
		if (typeof newText !== "string") throw new Error(`Expected newText to be a string`)
	...
	/**
	 * Minifies and returns the markup representation of the Leaf's __children__. The defaults to the options argument
	...
	innerMinify(options) {
		...
		return minify(serializeDOM(this.#getChildren({ convert: false })), options)
	}
	 ...
	 */
	select(query, n) {
		if (typeof query !== "string") throw new Error(`Expected query to be a string`)
	 ...
	 * Sets the text of Leaf.
	 *
	 * You can also explicitly set the text value using `(setter) text`.
	 *
	 * @param {String} newText
	 * @returns {Leaf}
	 */
