...
class Bunch {
	...
	/**
	 * Creates a Bunch - a collection of nodes representing elements or
	 * groups of elements from a markup language document.
	 *
	 * @param {Leaf[]|Bunch[]} leaves
	 * @returns {Bunch}
	 */
	constructor(leaves) {
		if (!Array.isArray(leaves)) throw new Error(`Expected leaves to be an array.`)
		...
		for (let i = 0; i < leaves.length; i++) {
			...
			if (!(leaf instanceof Leaf) && !(leaf instanceof Bunch))
				throw new Error(`Expected all in leaves to be an instance of a Leaf or Bunch.`)

			copy[copy.length] = leaf
		}
	 ...
	 */
	innerMinify(options) {
		return Promise.all(this.#leaves.map(async s => await s.innerMinify(options)).unwrap())
	}

	/**
	 * Loops over each Leaf in the Bunch, replacing the items with the return value of
	 ...
	 */
	map(callback) {
		return this.#leaves.map(callback).unwrap()
	}
	 ...
	 */
	minify(options) {
		return Promise.all(this.#leaves.map(async s => await s.minify(options)).unwrap())
	}
